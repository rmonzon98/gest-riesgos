/**
 * @fileoverview
 * Vista de consulta y reporte del Anexo 2 para superior.
 *
 * @module Riesgos/Anexo 2/Anexo2Superior.jsx
 * @version 1.1
 ...
 * Anexo2Superior
 *
 * Permite revisar por periodo el avance y estados del Anexo 2 consolidado.
 *
 ...
 * @returns {JSX.Element}
 */
export default function Anexo2Superior() {
    ...
    const comentarioBloqueado = useMemo(() => {
        const c = histSel?.COMENTARIO_SUPERVISOR ?? histSel?.COMENTARIO_SUPERIOR;
        return typeof c === 'string' && c.trim() !== '';
    }, [histSel]);

    // Controles de impresión (mismo enfoque que Anexo2Sup)
    const [printFilter, setPrintFilter] = useState('all'); // 'all' | 'indices'
    ...
    // Logo (base64) para PDF (mismo patrón que en Anexo2Sup)
    ...
    /**
     * Carga desde el backend el historial del Anexo 2 para el periodo indicado.
     */
    const cargarEstadoHistorial = async (selPeriodo) => {
        ...
        try {
            ...
            setEstadoActualInfo({
                ...
                FECHA_MODIFICACION: ultimo?.FECHA_SUPERIOR ?? null
            });

            const sinComent = lista.filter(h =>
                !h.COMENTARIO_SUPERIOR ||
                String(h.COMENTARIO_SUPERIOR).trim() === ''
            );
            ...
            if (Array.isArray(mats)) {
                ...
                setMatrices([]);
                setActive(0);
                setAlerta('No se encontró una respuesta para ese período.');
            }
        } catch (e) {
    ...
    const handleGuardarDecision = async () => {
        if (!periodo) { setAlerta('Seleccione un período antes de guardar.'); return; }
    ...
    // === Utilidades de impresión (copiadas/adaptadas de Anexo2Sup) ===
    ...
    const openPdfModal = async (opts) => {
        if (!histSel) {
            setAlerta('No hay historial seleccionado para imprimir.');
            return;
        }
        ...
        try {
            ...
            console.error('Error obteniendo info de superior para PDF', e);
    ...
    const confirmarImprimir = async () => {
        ...
        try {
            // No hay nombre de unidad aquí; usamos un texto genérico para nivel superior
            ...
            console.error('Error generando PDF Anexo 2 (superior)', e);
    ...
    const handleImprimir = async () => {
        if (!matrices?.length) {
            setAlerta('No hay tablas para imprimir.');
            return;
        }
        ...
        <Box sx={{ p: 2 }}>
            ...
            {periodo && (
                <Card sx={{ borderRadius: 2 }}>
                    <CardContent>
                        {matrices.length === 0 ? (
                            <Typography variant="body2">No hay respuesta guardada para este período.</Typography>
                        ) : (
                            <>
...

                                {
                                    ...
                                    <>
                                        ...
                                        {alerta && (
                                            <Alert severity="info" sx={{ mt: 2 }} onClose={() => setAlerta(null)}>
                                                {alerta}
                                            </Alert>
                                        )}
                                    </>
            ...
            {/* Modal de confirmación antes de imprimir (igual que en Anexo2Sup) */}
            <Dialog
                open={pdfModalOpen}
